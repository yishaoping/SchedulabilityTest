...
double WCET_Ratio[Policy_number][TaskType_number] = {
    { 1, 1, 1, 1.001, 1, 1.003, 1, 1 },  //PMC
    { 1.382, 2.404, 3.587, 1.905, 2.304, 2.495, 2.679, 1.715 },  //AS
    { 1.004, 1.012, 1.008, 1.005, 1, 1.012, 1.001, 1.012 },  //SS
};
...
/* 主函数 */
void Set(System* system, int area, int total_BigCore_number, int total_task_number, double core_rate);
...
int main()
{
    ...
    for (int i = 0; i < 7; i++)
    {
        ...
        for (int k = 0; k < 5; k++)
        {
            ...
            {
                Set(&system, area, total_BigCore_number, task_number, l);
                Run(&system);
                fflush(stdout);
            }
        }
    ...
    for (int i = 0; i < Policy_number; i++)
    {
        for (int j = 0; j < TaskType_number; j++)
        {
            ...
            while (k) {
                ...
                // 基本参数
                tasks[count].wcet = WCET[j] * WCET_Ratio[i][j];
...
void Write2CSV(System* system, const char* fileid)
{
    const char* header = "Area, BigCore Number, Task Number, Per Rate, Schedulability, ErrorType\n";
