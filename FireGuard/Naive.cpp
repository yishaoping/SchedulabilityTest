...
typedef struct task
{
    ...
    int policy = -1;
    int task_type = -1;
    int littleCore_num_max = 0;
} Task;

typedef struct
{
    Task* current_task;
    int littleCore_number;
} BigCore;
...
void Distribution(System* system);  // 重新分配小核

void EnqueueTask(Task* queue, Task* task);  // 入队出队
...
int CompareD(const void* a, const void* b)
{
    // 传入的是指向 BigCore指针 的指针
    BigCore* coreA = *(BigCore**)a;
    BigCore* coreB = *(BigCore**)b;

    // 空闲核心优先级最低（排在最后）
    if (!coreA->current_task && !coreB->current_task) return 0;
    if (!coreA->current_task) return 1;  // A空闲，A放后面，返回1
    if (!coreB->current_task) return -1; // B空闲，B放后面，返回-1

    long long int ddlA = coreA->current_task->deadline;
    long long int ddlB = coreB->current_task->deadline;

    if (ddlA < ddlB) return -1;
    if (ddlA > ddlB) return 1;
    return 0;
}
void Distribution(System* system)
{
    // --- 准备工作：创建指针数组以避免破坏原数组结构 ---
    BigCore** core_ptrs = (BigCore**)malloc(sizeof(BigCore*) * system->total_BigCore_number);
    if (!core_ptrs) return; // 错误处理

    for (int i = 0; i < system->total_BigCore_number; i++) {
        core_ptrs[i] = &system->cores[i];
        // 重置所有核的小核计数，防止累加错误
        core_ptrs[i]->littleCore_number = 0;
    }

    // --- 排序 ---
    qsort(core_ptrs, system->total_BigCore_number, sizeof(BigCore*), CompareD);

    int free_little_cores = system->total_LittleCore_number;

    // --- 第一阶段：保底分配 (每人至少拿1个，直到分完) ---
    // 此时数组已排序，优先给紧急任务分配
    for (int i = 0; i < system->total_BigCore_number; i++) {
        BigCore* core = core_ptrs[i];
        if (core->current_task && free_little_cores > 0) {
            core->littleCore_number = 1;
            free_little_cores--;
        }
    }
    ...
    // --- 第二阶段：贪婪分配 (填满最大需求) ---
    // 如果还有剩余小核，继续分给最紧急的任务
    if (free_little_cores > 0) {
        for (int i = 0; i < system->total_BigCore_number; i++) {
            BigCore* core = core_ptrs[i];
            Task* ctask = core->current_task;

            // 遇到空闲核，由于排序过，后面肯定都是空闲核，直接退出
            if (!ctask) break;

            int need = ctask->littleCore_num_max - 1;
            if (free_little_cores >= need) {
                core->littleCore_number += need;
                free_little_cores -= need;
            }
            else {
                core->littleCore_number += free_little_cores;
                free_little_cores = 0;
                break;
            }
        }
    ...
    for (int i = 0; i < Policy_number; i++)
    {
        for (int j = 0; j < TaskType_number; j++)
        {
            ...
            while (k) {
                ...
                tasks[count].littleCore_num_max = LittleCore_Max[i][j];
...
void Execute(System* system)
{
    ...
    for (; system_time < system->maxtime; system_time++)
    {
        ...
        for (int c = 0; c < system->total_BigCore_number; c++) {
            ...
            if (ctask) {
                double work = 1 / WCET_Ratio[ctask->policy][ctask->task_type][core->littleCore_number];
                ctask->workload -= work;
...
void Initialization(System* system)
{
    ...
    for (int i = 0; i < system->total_BigCore_number; i++)
    {
        ...
        system->cores[i].littleCore_number = 0;
    }
